...
- `src/app/` is the application, `packages/*` are shared libraries. The
  template ships no packages — never assume one exists.
...
- Configuration goes through `src/app/config.py`, never `os.environ`.
